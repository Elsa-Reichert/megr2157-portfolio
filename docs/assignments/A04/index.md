...
## Objective
The objective for this project is to design a motor mount for this motor that attaches to a rigid wall: [link to motor](https://www.omc-stepperonline.com/brushed-24v-dc-gear-motor-3-6kg-cm-46rpm-w-99-5-1-planetary-gearbox-pa28-28245800-g100). After researching motor mounts, I will design for yield strenth then for a maximium deflect at the free end. I am allowed to select betweem ABS, PETG, or PLA for the material. 

Picture of the motor
<img width="1625" height="505" alt="image" src="https://github.com/user-attachments/assets/830cd35f-3702-417e-bed3-d004bec447d1" />
...
### Feature 1
My first task is to design a feature of the mount that is attached to the motor. The deflection of the feature is zero and the derivative with respect to x is also zero, which means I can treat the feature as a cantilever beam while solving for the cross sectional area. The safety factory accounts for the holes and skrews in the motor mount. I need to determind the cross sectional area geometry using the equations for both stress and deflection. 

Givens: P=300, Yield stress = 51.6 MPa, E = 3131 MPa, Max. deflection = .30 mm, Safety factor = 3

I decided to use PLA for the building matrial. The source I used gave a range for both the yield strength and Youngs Modulus. I am using the lowest posible value just to be safe. 

#### Bending

We are given a force p that is applied to the shaft of the motor. My first step is to figure out the moment on the end of feature one. Force P is the only force acting because we are neglecting the weight of the motor. I need to find the vertical distance of E and the force vector P. Looking at the diagram given on the motors manufactoring webside, the shaft is about 18mm. 

My first thought was to have the shaft stick out intirely and add half the height of the beam. The issue with this was I would have an unknown verrible in my moment equation and a lot of the motor mounts have space for the shaft to stick into and stay in place. 
...
I realized the moment at the top end of the beam would be the highest. So I went back and changed the vertical distance from P value to 22mm. This extra height gives more space for the motor to sit into this feature, as while as gives me the largest postible moment calculation. 
...
#### Max. Stress
I first started using the max. stress. I used a rectangular cross section the first time I tried to solve it and realized I would need a third equaction. Here was the math for that.
...
### Feature 2
I need to design a feature of the mount to be attached to the wall. I can assume the rigid wall A can support bolts. I will be determining the cross sectional gemoetry using the same method.

#### Moment
The moment for this feature could be different because it is at a different location. My first step is to find the moment acting at the bottom corner. 
...
## Decide
This design was created to hold a motor. I choose PLA because it had the largest yield strength and Youngs modulus value. The geometry was determined using the bending motion equations. The area is large enough to make sure the stress over the yield strength with a safety factor of 3. The Length of the structure is also right to make sure the features do not go over the max deflection. 
