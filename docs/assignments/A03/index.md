...
## Objective
My objective for this project is to design a bar using a circular cross section where the values of the criteria are given for the material, maximum deflection, and load. I then need to determine the bars minimum geometry through parametric deign while under direct tension. Then verify the gemotry through finite element analysis. 

The purpose of this assignment is to learn how to use parametric design to determine a beams geomtetry and learn how to use finite element analysis to verify that geomtry
...
To do this I used the equations feature in solidworks and made a variable for the radius of the cross section, the area of the cross section,the force applied to the bar, the max selection, youngs modiolus of the material (Al), and the length of the bar. This allowed me to change any value very quickly, because now the CAD program is using that variable to calculate different values and variables for the geometric of the struture. This means if i needed to change the radius, instead of changing the values in my drawing and recalculating every equation, I just need to change the radius variable.

Because I am given a range for the force and Youngs Mognousgs, this approch allows me to change either of those values within that range to see what the structure will do. It also makes it more convieiont to change the gemotry if needed. This helped me a lot because I ended up changing the radius 3 times.  

My first set of variables written out in solidworks, value for the cross sectional area, and the length of the bar determinded parametrically.
...
The equauation for the axial deformation is given in the mercheringofs text bopo as 
...
Which can be used to find the length. 
![solved for L](https://github.com/Elsa-Reichert/megr2157-portfolio/blob/main/docs/assignments/A03/solved%20for%20L.jpg)

I did not put much thought into the radius. This was a mistake I made twice. You can see my first r value was 1 in. Which would make L equal about 400-1000 in depending on the Force value and Young's Modulus Value. I descided to change it to (change) to make it more practical. Here is a photo of the oringal r value strucutre.

![r=1,l=800](https://github.com/Elsa-Reichert/megr2157-portfolio/blob/main/docs/assignments/A03/Lmax%20400%2B%20in.jpg)

Here are my reviesed values.
...
Using this radius value; r = .05. I expiramented with the force and young's modulus values to get the longest and shortest bar. The bar's length is this longest when the force is at its maximum value and young's modulus is at its minium value. This makes sense if you look at the axial defomration equatoin. After solving for L, F is at the bottom and E is at the top, which means when F gets bigger, L gets smaller and when E gets larger, L gets larger. 
...
After I was happy with my design, I needed to use this drawing to conduct a FEA bar using the same force load used to calculate the bars gemotry. To do this, I ran a simulation on my drawing in solidworks with the material set as 6061-T6 alummanium. 
...
Then generated a von Mises stress map, delectrion map, and Strain. Please note the graphs are in SI still. 
...
First I had to calculate the minium requred area using the stress equation. I rounded the yield strength value given by solid works to 40x10^3. I also added a safety factor of 3.
...
Because the bar was parametrically designed, all I had to do was go into the equations tab and change r to .09in. It was rounded to .09 because solidworks is set to 2 decimals by defualt. I did not feel the need to change it because .085 was a minimum value and I felt the values were precise enough.  
